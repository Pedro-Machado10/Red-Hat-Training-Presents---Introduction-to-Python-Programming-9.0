...
Este módulo aborda as quatro estruturas de dados fundamentais: Listas, Tuplas, Conjuntos (Sets) e Dicionários, além de ordenação.
...
| :--- | :--- | :--- |
| `list()` | Cria uma lista vazia ou converte iterável em lista. | `list("abc")` → `['a','b','c']` |
| `tuple()` | Cria uma tupla vazia ou converte iterável em tupla. | `tuple([1, 2])` → `(1, 2)` |
| `set()` | Cria um conjunto vazio ou converte iterável (remove duplicatas). | `set([1, 2])` → `{1, 2}` |
| `dict()` | Cria um dicionário vazio ou converte lista de pares chave-valor. | `dict([(k,v)])` |
| `enumerate()` | Retorna objeto enumerado (índice, valor) durante loops. | `for i, v in enumerate(lista):` |
| `sorted()` | Função built-in que retorna uma **nova** lista ordenada. | `nova_lista = sorted(lista)` |

### Métodos de Lista (Mutáveis e Ordenadas)
| Método | Descrição |
| :--- | :--- |
...
### Métodos de Conjunto/Set (Não Ordenados e Únicos)
| Método | Descrição |
| :--- | :--- |
...
### Métodos de Dicionário (Chave-Valor)
| Método | Descrição |
| :--- | :--- |
...
### Operadores Matemáticos de Conjuntos (Sets)
| Op | Nome | Descrição |
| :--- | :--- | :--- |
